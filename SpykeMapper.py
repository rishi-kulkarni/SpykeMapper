...
def digitizeSpikes(spikearray):
    ...
    thresh = (
        6 * (pd.Series(spikearray).rolling(250).std().min())
        + pd.Series(spikearray).rolling(500).median().median()
    )
...
def plotBaseline(BaseNormal):
    ...
    for i in range(0, np.ma.size(BaseNormal, 0) - 1):
        plt.plot(
            ...
            i
            + (
                (BaseNormal.values[:, i] - np.min(BaseNormal.values[:, i]))
                / (np.max(BaseNormal.values[:, i]) - np.min(BaseNormal.values[:, i]))
            ),
            label="baseline",
        )
...
def spike_triggered_baseline(normalized_baseline, spiketrain):
    ...
    for cellnum in range(0, np.max(markers[1]) + 1):
        ...
        for spiketime in markers[:, indices[0]][0]:
            ...
            spike_trigger_baseline[:, cellnum] = (
                spike_trigger_baseline[:, cellnum] + toadd
            )
        np.divide(
            spike_trigger_baseline[:, cellnum],
            (np.count_nonzero((markers[1, :] == cellnum))),
            out=spike_trigger_baseline[:, cellnum],
            where=(np.count_nonzero((markers[1, :] == cellnum))) != 0,
        )
...
def generate_ISI(spiketrain, subthresh):
    ...
    for i in range(0, len(spiketrain.values[0, :])):
        for j in range(0, np.ma.size(np.where(spiketrain.iloc[:, i])[0]) - 1):
            ISI_baseline = np.append(
                ISI_baseline,
                np.mean(
                    subthresh.iloc[
                        np.where(spiketrain.iloc[:, i])[0][j] : np.where(
                            spiketrain.iloc[:, i]
                        )[0][j + 1],
                        i,
                    ]
...
class VoltageTraceData:
    ...
    def process_raw(self, smooth=5000, weight=0.01, iterat=10):
        self.SubThreshold = self.RawData.transform(
            baseline_als, axis=0, raw=True, lam=smooth, p=weight, niter=iterat
        )
    ...
    def gen_STA(self):
        ...
        self.STA[:] = spike_triggered_baseline(
            self.NormSubThresh.values, self.SpikeTrain.values
        )
